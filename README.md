...
- Signup email verification and password-reset emails use Brevo's HTTPS transactional email API in production when `BREVO_API_KEY` is set; otherwise they fall back to Django SMTP settings. In local development without those values, mail is written to the console.
